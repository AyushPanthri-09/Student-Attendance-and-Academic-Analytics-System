...
export default function RiskAnalytics() {
  ...
  const fetchRiskData = async () => {
    try {
      ...
      const data = await analyticsService.getRiskAnalysis(currentPage - 1, pageSize, searchTerm || null, riskParam || null);
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {paginatedData.map((student) => (
                <tr key={student.studentId} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getRiskColor(student.riskCategory)}`}>
                      {getRiskIcon(student.riskCategory)}
                      <span className="ml-1">{student.riskCategory}</span>
                    </span>
                  </td>
        ...
        {totalItems > pageSize && (
          <div className="flex items-center justify-between p-4 border-t border-gray-100">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={async () => { setCurrentPage(prev => Math.max(prev - 1, 1)); await fetchRiskData(); }}
              ...
              <button
                onClick={async () => { setCurrentPage(prev => Math.min(prev + 1, totalPages)); await fetchRiskData(); }}
