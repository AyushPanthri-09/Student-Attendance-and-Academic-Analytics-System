...
public interface StudentRepository extends JpaRepository<Student, Long> {
    ...
    @Query("SELECT s.id as id, s.name as name, s.rollNo as rollNo, s.department.name as departmentName, s.semester as semester, s.examEligible as examEligible, s.department.id as departmentId FROM Student s")
    List<StudentProjection> findAllStudentProjections();
    
    @Query("SELECT s.id as id, s.name as name, s.rollNo as rollNo, s.department.name as departmentName, s.semester as semester, s.examEligible as examEligible, s.department.id as departmentId FROM Student s WHERE s.department.id = :departmentId")
